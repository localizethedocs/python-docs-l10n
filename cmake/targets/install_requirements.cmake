...
if (CURRENT_REFERENCE MATCHES "(rc[1-9][0-9]*)$")
    set(CONDA_SPEC_OF_PYTHON    conda-forge::python<=${VERSION_OF_PYTHON})
else()
    set(CONDA_SPEC_OF_PYTHON    conda-forge::python=${VERSION_OF_PYTHON})
endif()
remove_cmake_message_indent()
message("")
message("CONDA_SPEC_OF_PYTHON = ${CONDA_SPEC_OF_PYTHON}")
message("")
execute_process(
    COMMAND ${Conda_EXECUTABLE} install
            ${CONDA_SPEC_OF_PYTHON}
            conda-forge::pip=${VERSION_OF_PIP}
            --channel conda-forge
            --prefix ${PROJ_CONDA_DIR}
            --yes
    ECHO_OUTPUT_VARIABLE
...
if (  ( VERSION MATCHES "^main$" ) OR
      ( VERSION VERSION_LESS_EQUAL    "3.14" AND
        VERSION VERSION_GREATER_EQUAL "3.8" )  )
    ...
    message(STATUS "Running 'pip install' command to install the dependencies...")
    if (CMAKE_HOST_LINUX)
        set(ENV_PATH                "${PROJ_CONDA_DIR}/bin:$ENV{PATH}")
        set(ENV_LD_LIBRARY_PATH     "${PROJ_CONDA_DIR}/lib:$ENV{ENV_LD_LIBRARY_PATH}")
        set(ENV_VARS_OF_SYSTEM      PATH=${ENV_PATH}
    ...
    # Refer to https://github.com/python/docsbuild-scripts/blob/213530446b083788da9dbf4ca02284b5dc9c5c5c/build_docs.py#L101
    if     (  ( VERSION VERSION_LESS_EQUAL    "3.7" AND
                VERSION VERSION_GREATER_EQUAL "3.5" ) OR
              ( VERSION VERSION_LESS_EQUAL    "2.7" AND
                VERSION VERSION_GREATER_EQUAL "2.7" )  )
        set(PYTHON_PACKAGE_LIST     sphinx==2.3.1
                                    docutils<=0.17.1
                                    jinja2<=3.0.3
                                    python-docs-theme==2021.5)
    elseif (  ( VERSION VERSION_LESS_EQUAL    "3.4" AND
                VERSION VERSION_GREATER_EQUAL "3.0" ) OR
              ( VERSION VERSION_LESS_EQUAL    "2.6" AND
                VERSION VERSION_GREATER_EQUAL "2.6" )  )
        set(PYTHON_PACKAGE_LIST     sphinx==1.8.4
                                    docutils<=0.17.1
                                    jinja2<=3.0.3
                                    python-docs-theme==2018.2)
    else()
        message(FATAL_ERROR "Invalid VERSION value. (${VERSION})")
    endif()
    remove_cmake_message_indent()
    message("")
    message("PYTHON_PACKAGE_LIST = ${PYTHON_PACKAGE_LIST}")
    message("")
    execute_process(
        COMMAND ${CMAKE_COMMAND} -E env
                ${ENV_VARS_OF_SYSTEM}
